...
import { Module, Trabajador, User } from '@/types';

// Definir la URL base para los trabajadores
const API_URL = '/users'; // Cambia la URL según tu API
...
export const updateUser = async (user: User, id: number): Promise<void> => {
  try {
    await axiosInstance.put(`${API_URL}/${id}`, user);
  } catch (error: any) {
